...
import { CheckCircle, MessageCircle, ArrowRight, Download, Clock, X, AlertCircle } from "lucide-react"
import { toast } from "sonner"

const REGISTRATION_FEE_PAYSTACK = 50 // ₵50 for Paystack payment
...
export default function PaymentSuccessPage() {
  ...
  useEffect(() => {
    ...
    if (date) {
      setRegistrationDate(decodeURIComponent(date))
    } else {
      setRegistrationDate(new Date().toLocaleString("en-US", {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
        hour: "2-digit",
  ...
  // Auto-open WhatsApp to admin when user lands after Paystack payment - so admin gets notified immediately
  useEffect(() => {
    const id = searchParams.get("agentId")
    const ref = searchParams.get("reference")
    const name = searchParams.get("agentName") ? decodeURIComponent(searchParams.get("agentName")!) : "Agent"
    const mailParam = searchParams.get("email") ? decodeURIComponent(searchParams.get("email")!) : ""
    if (!id || !ref) return
    const storageKey = `paystack_success_whatsapp_${id}_${ref}`
    if (typeof window !== "undefined" && !sessionStorage.getItem(storageKey)) {
      const timer = setTimeout(() => {
        ...
        const activationMessage = `✅ *AGENT REGISTRATION - PAYSTACK PAYMENT CONFIRMED*

Hello Dataflex Admin,

I have successfully completed my agent registration payment via Paystack and would like to request account activation.
...
*What's Next:*
Please contact us on WhatsApp to request account activation. Once approved, you'll be able to:
...
• Payment Reference: ${reference}
• Amount Paid: ₵${REGISTRATION_FEE_PAYSTACK}.00
• Payment Date & Time: ${registrationDate || timestamp}
• Status: ✅ Payment Verified
...
${agentName}`
    ...
    toast.success("WhatsApp opened! Send the message to admin for faster activation")
  }
          ...
          <Card className="shadow-2xl border-0 mb-6">
            ...
            <CardContent className="pt-8 space-y-6">
              ...
              <div className="bg-gradient-to-r from-emerald-100 to-green-100 rounded-lg p-6 border border-emerald-200">
                ...
                <p className="text-4xl font-bold text-emerald-600">₵{REGISTRATION_FEE_PAYSTACK}.00</p>
              ...
              <div className="space-y-3 pt-4">
                <Button
                  onClick={handleContactAdminForActivation}
                  disabled={contactAdminLoading}
                  className="w-full h-14 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white font-bold text-lg rounded-lg shadow-lg transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-70"
                >
                  <MessageCircle className="h-5 w-5" />
                  {contactAdminLoading ? "Opening WhatsApp..." : "📲 Contact Admin for Activation"}
                </Button>

                <p className="text-xs text-center bg-emerald-50 text-emerald-800 py-2 px-3 rounded-lg border border-emerald-200">
                  ✓ Your payment is verified. Click above to request account activation via WhatsApp!
                </p>
              </div>
            </CardContent>
          </Card>
