...
export default function DataOrdersPage() {
  ...
  const generateEnhancedCSV = (transactionFlow: any[], orders: DataOrder[]) => {
    const reportSummary = [
      ...
      `Total Available Commission: GH₵ ${commissionData.totalAvailableCommissions.toFixed(2)}`,
      `Report Period: ${transactionFlow.length > 0 ? format(new Date(transactionFlow[0].timestamp), "yyyy-MM-dd") : "N/A"} to ${transactionFlow.length > 0 ? format(new Date(transactionFlow[transactionFlow.length - 1].timestamp), "yyyy-MM-dd") : "N/A"}`,
      "",
    ...
    const orderData = orders.map((order) => {
      ...
      return [
        ...
        (order as any).bundle_status || "valid",
        verificationHash,
      ]
    })
        ...
        <div className="container mx-auto px-4 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white drop-shadow-lg">My Data Orders</h1>
