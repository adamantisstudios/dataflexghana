...
export async function DELETE(request: Request) {
  try {
    ...
    // Verify the order exists and check status
    const { data: existing } = await supabase.from("bulk_orders").select("agent_id, status").eq("id", id).single()

    if (!existing) {
      return Response.json({ error: "Bulk order not found" }, { status: 404 })
    }

    if (existing.status !== "pending" && existing.status !== "pending_admin_review") {
      return Response.json({ error: "Can only delete pending bulk orders" }, { status: 403 })
    }
    ...
    console.error("Error in agent delete bulk order route:", error)
