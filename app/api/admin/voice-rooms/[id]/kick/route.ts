...
export async function POST(
  ...
  try {
    ...
    await getLiveKitRoomService().removeParticipant(room.room_name, identity)
    return NextResponse.json({ success: true })
  } catch (e) {
    console.error("[admin/voice-rooms/kick]", e)
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Failed to kick participant" },
      { status: 500 },
    )
  }
}
