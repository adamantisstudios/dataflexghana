...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!String(before.profile_image_url ?? "").trim()) {
      return NextResponse.json({ error: "Agent has no profile photo" }, { status: 400 })
    }
    ...
    console.error("[admin/photo-verification/approve]", e)
