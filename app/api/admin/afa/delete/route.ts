...
export async function DELETE(request: Request) {
  try {
    ...
    // Verify the registration exists and check its status before deleting
    ...
    if (fetchError || !existing) {
      console.error("Error fetching AFA registration:", fetchError)
    ...
    if (existing.status !== "completed" && existing.status !== "canceled") {
      return Response.json(
        { error: "Can only delete completed or canceled AFA registrations" },
        { status: 403 }
      )
    }
    ...
    console.error("Error in admin delete AFA route:", error)
