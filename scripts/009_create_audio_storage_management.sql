-- Phase 3: Storage quota and cleanup system
...
CREATE TRIGGER trigger_update_audio_storage
...
BEGIN
  ...
  LOOP
    ...
    SET total_size_bytes = total_size_bytes - v_file.file_size_bytes,
...
CREATE POLICY "Users can view their own storage usage"
  ON audio_storage_usage FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can view their own audio metadata"
  ON audio_files_metadata FOR SELECT
  USING (auth.uid() = user_id);
