...
interface DashboardLoginNotificationProps {
  agentId: string
}

export default function DashboardLoginNotification({ agentId }: DashboardLoginNotificationProps) {
  ...
  useEffect(() => {
    ...
    const checkDisplayEligibility = () => {
      const today = new Date().toDateString()
      const storageKey = `dashboardNotification_${agentId}`
      const stored = localStorage.getItem(storageKey)
      
      let displayData = { date: today, count: 0 }
      ...
      if (stored) {
        try {
          ...
          console.error("Error parsing notification display data:", error)
        }
      }
      
      // Show notification if count is less than 3 for today
    ...
    const timer = setTimeout(checkDisplayEligibility, 1000)
