...
interface TutorialCommentsPanelProps {
  videoId: string
  isOpen: boolean
  onClose: () => void
}
...
export function TutorialCommentsPanel({ videoId, isOpen, onClose }: TutorialCommentsPanelProps) {
  ...
  const loadComments = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/agent/tutorial-comments?videoId=${encodeURIComponent(videoId)}`, {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch("/api/agent/tutorial-comments", {
        method: "POST",
        headers: { ...getAgentAuthHeaders(), "Content-Type": "application/json" },
        body: JSON.stringify({ videoId, content: content.trim() }),
      })
      const data = await res.json()
...
export default TutorialCommentsPanel
