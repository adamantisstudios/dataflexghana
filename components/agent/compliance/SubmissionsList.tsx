                  ...
                  <div className="space-y-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-gray-900 truncate">{formName}</h4>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                      <Badge className={`${getStatusColor(submission.status)} border justify-center sm:justify-start`}>
                        {submission.status}
                      </Badge>
                      <Button
                        variant="outline"
      ...
      <Dialog open={showViewDialog} onOpenChange={setShowViewDialog}>
        <DialogContent className="max-w-[95vw] sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>View the details of your compliance form submission</DialogDescription>
          </DialogHeader>
          ...
          {selectedSubmission && (
            <div className="space-y-4 mt-4">
              ...
              <div>
                <span className="font-semibold text-lg">Form Information</span>
                <div className="mt-3 space-y-3">
                  {formatFormData(selectedSubmission.form_data).map((field, index) => (
                    <div key={index} className="border-b border-gray-200 pb-2">
                      <p className="text-sm font-medium text-gray-700">{field.label}</p>
                      <p className="text-sm text-gray-900 mt-1 break-words">{field.value}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {selectedSubmission && (
        <Dialog open={showChatDialog} onOpenChange={setShowChatDialog}>
          <DialogContent className="max-w-[95vw] md:max-w-2xl max-h-[90vh] flex flex-col">
            <DialogHeader>
              <DialogTitle className="text-lg md:text-xl font-bold text-blue-800">
