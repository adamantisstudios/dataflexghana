...
interface PartnerData {
  ...
  ghanaCardBackFile?: File | null
  ghanaCardBackPreview?: string | null
}
...
export function PartnershipForm({ agentId, onComplete, onCancel }: PartnershipFormProps) {
  ...
  const handlePartnerGhanaCardChange = (index: number, file: File | null, side: "front" | "back" = "front") => {
    if (!file) return
    ...
    reader.onloadend = () => {
      if (side === "front") {
        ...
        updatePartnerData(index, "ghanaCardBackFile", file)
        updatePartnerData(index, "ghanaCardBackPreview", reader.result as string)
      }
    }
    reader.readAsDataURL(file)
  }

  const removePartnerGhanaCard = (index: number, side: "front" | "back" | "all" = "all") => {
    ...
    if (side === "back" || side === "all") {
      updatePartnerData(index, "ghanaCardBackFile", null)
      updatePartnerData(index, "ghanaCardBackPreview", null)
    }
  }
    ...
    try {
      ...
      for (let i = 0; i < formData.partners.length; i++) {
        ...
        if (partner.ghanaCardFile) {
          const ghanaCardUrl = await uploadImage(partner.ghanaCardFile, "ghana_card_front", i)
          if (ghanaCardUrl) {
            imageInserts.push({
              submission_id: finalSubmissionId,
        ...
        if (partner.ghanaCardBackFile) {
          const ghanaCardBackUrl = await uploadImage(partner.ghanaCardBackFile, "ghana_card_back", i)
          if (ghanaCardBackUrl) {
            imageInserts.push({
              submission_id: finalSubmissionId,
                ...
                <div key={index} className="bg-emerald-50 rounded-lg p-4 border border-emerald-200">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="mt-3 space-y-3">
                      ...
                      <div className="space-y-4">
                        <div>
                          <Label className="text-base font-semibold">Ghana Card - Front *</Label>
                          <div className="border-2 border-dashed border-emerald-300 rounded-lg p-3 w-full mt-1">
                            {partner.ghanaCardPreview ? (
                              <div className="relative">
                                ...
                                <Button
                                  ...
                                  onClick={() => removePartnerGhanaCard(index, "front")}
                                >
                                  <X className="h-4 w-4" />
                                </Button>
                              </div>
                              ...
                              <label className="flex flex-col items-center cursor-pointer py-6 w-full">
                                ...
                                <input
                                  ...
                                  accept="image/*"
                                  className="hidden"
                                  onChange={(e) => handlePartnerGhanaCardChange(index, e.target.files?.[0] || null, "front")}
                                />
                              </label>
                            )}
                          </div>
                        ...
                        <div>
                          <Label className="text-base font-semibold">Ghana Card - Back *</Label>
                          <div className="border-2 border-dashed border-emerald-300 rounded-lg p-3 w-full mt-1">
                            {partner.ghanaCardBackPreview ? (
                              <div className="relative">
                                <img
                                  src={partner.ghanaCardBackPreview || "/placeholder.svg"}
                                  alt="Ghana Card Back"
                                  className="max-h-48 mx-auto rounded w-full object-contain"
                                />
                                <Button
                                  ...
                                  size="sm"
                                  className="absolute top-2 right-2"
                                  onClick={() => removePartnerGhanaCard(index, "back")}
                                >
                                  <X className="h-4 w-4" />
                                </Button>
                              </div>
                              ...
                              <label className="flex flex-col items-center cursor-pointer py-6 w-full">
                                ...
                                <input
                                  ...
                                  accept="image/*"
                                  className="hidden"
                                  onChange={(e) => handlePartnerGhanaCardChange(index, e.target.files?.[0] || null, "back")}
                                />
                              </label>
                            )}
                          </div>
