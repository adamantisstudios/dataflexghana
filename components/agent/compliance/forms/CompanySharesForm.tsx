...
export function CompanySharesForm({ agentId, onComplete, onCancel }: CompanySharesFormProps) {
  ...
  const formFee = amount(SERVICE_PRICING_KEYS.COMPLIANCE_COMPANY_SHARES, 1930)
  const commission = amount(SERVICE_PRICING_KEYS.COMPLIANCE_COMPANY_SHARES_COMMISSION, 70)
  ...
  const [personGhanaCards, setPersonGhanaCards] = useState<Record<string, { file: File; preview: string; backFile?: File; backPreview?: string }>>({})
  ...
  const handleGhanaCardChange = (person: string, file: File | null, side: "front" | "back" = "front") => {
    if (!file) return
    ...
    reader.onloadend = () => {
      setPersonGhanaCards((prev) => {
        const current = prev[person] || { file: new File([], ""), preview: "" }
        if (side === "front") {
          return {
            ...prev,
            [person]: { ...current, file, preview: reader.result as string },
          }
        } else {
          return {
            ...prev,
            [person]: { ...current, backFile: file, backPreview: reader.result as string },
          }
        }
      })
    }
  ...
  const removeGhanaCard = (person: string, side: "front" | "back" | "all" = "all") => {
    setPersonGhanaCards((prev) => {
      if (side === "all") {
        const updated = { ...prev }
        ...
        return {
          ...prev,
          [person]: { ...current, file: new File([], ""), preview: "" },
        }
      } else {
        const current = prev[person]
        if (!current) return prev
        return {
          ...prev,
          [person]: { ...current, backFile: undefined, backPreview: undefined },
        }
      }
    })
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const personIndexMap: Record<string, number> = {
        "director1": 0,
        "director2": 1,
        "secretary": 2,
        "subscriber1": 3,
      ...
      for (const person of people) {
        const personIndex = personIndexMap[person]
        ...
        if (personGhanaCards[person]) {
          ...
          if (personGhanaCards[person].file) {
            const ghanaCardFrontUrl = await uploadImage(personGhanaCards[person].file, "ghana_card_front", person)
            if (ghanaCardFrontUrl) {
              uploadedImages.push({
                submission_id: "",
          ...
          if (personGhanaCards[person].backFile) {
            const ghanaCardBackUrl = await uploadImage(personGhanaCards[person].backFile, "ghana_card_back", person)
            if (ghanaCardBackUrl) {
              uploadedImages.push({
                submission_id: "",
      ...
      onComplete({ cost: formFee, formName: "Company Limited By Shares" })
    } catch (error: any) {
          ...
          <CardContent className="space-y-4">
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-blue-600">{formFee.toLocaleString()} GHS</span>
              </div>
              ...
              <div className="space-y-6 w-full">
                ...
                {["director1", "director2", "secretary", "subscriber1", "subscriber2"].map((person) => (
                  <div key={person} className="border-t pt-6 mt-4">
                    ...
                    {/* Ghana Card Upload Section - Both Front and Back */}
                    <div className="mb-6 space-y-4">
                      <Label className="text-sm font-medium block">Ghana Card - Front *</Label>
                      <div className="w-full border-2 border-dashed border-blue-300 rounded-lg p-4">
                        {personGhanaCards[person]?.preview ? (
                          <div className="relative">
                            <div className="w-full h-48 flex items-center justify-center mb-4">
                              <img
                                src={personGhanaCards[person].preview}
                                alt={`${person} Ghana Card Front`}
                                className="max-h-full max-w-full object-contain"
                              />
                            </div>
                            <Button
                              ...
                              onClick={() => removeGhanaCard(person, "front")}
                            >
                              <X className="h-4 w-4" />
                            </Button>
                          </div>
                      ...
                      <Label className="text-sm font-medium block">Ghana Card - Back *</Label>
                      <div className="w-full border-2 border-dashed border-blue-300 rounded-lg p-4">
                        {personGhanaCards[person]?.backPreview ? (
                          <div className="relative">
                            <div className="w-full h-48 flex items-center justify-center mb-4">
                              <img
                                src={personGhanaCards[person].backPreview}
                                alt={`${person} Ghana Card Back`}
                                className="max-h-full max-w-full object-contain"
                              />
                            </div>
                            <Button
                              ...
                              size="sm"
                              className="absolute top-2 right-2"
                              onClick={() => removeGhanaCard(person, "back")}
                            >
                              <X className="h-4 w-4" />
                            </Button>
                          </div>
