...
export function PassportForm({ agentId, onComplete, onCancel }: PassportFormProps) {
  ...
  <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-3">
    <Card className="w-full max-w-sm sm:max-w-xl md:max-w-2xl border-blue-300 bg-white shadow-2xl max-h-[95vh] overflow-y-auto">
      <CardHeader className="p-3 sm:p-5 relative border-b border-blue-100">
        <div className="flex items-start gap-2 pr-8">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0">
            <FileText className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600" />
          </div>
          <div>
            <CardTitle className="text-base sm:text-lg text-blue-600">Passport Processing</CardTitle>
            <p className="text-xs text-gray-600 mt-0.5">Select your preferred speed</p>
          </div>
        </div>
        {/* Close button */}
        <button
      ...
      <CardContent className="space-y-3 p-3 sm:p-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
          {PASSPORT_COST_TIERS.map((tier) => (
            ...
            >
              <h4 className="font-bold text-sm text-blue-800">{tier.description}</h4>
              <p className="text-xs text-gray-600 font-medium mt-1">{tier.days}</p>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-2">
          <p className="text-xs text-gray-700">
            <span className="font-semibold text-blue-800">Includes:</span> processing, registration, and nationwide delivery.
          </p>
        </div>

        <Button
          onClick={() => setShowCostPopup(false)}
          className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 text-sm"
        ...
        <CardContent className="space-y-6 [&_input]:min-h-11 [&_input]:focus-visible:ring-2 [&_input]:focus-visible:ring-green-500 [&_input]:focus-visible:ring-offset-0 [&_textarea]:focus-visible:ring-2 [&_textarea]:focus-visible:ring-green-500 [&_textarea]:focus-visible:ring-offset-0 [&_button]:min-h-11" data-form-section>
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-3 pt-6 border-t">
            <Button
              ...
              variant="outline"
              className="w-full sm:w-auto flex items-center gap-2 border border-gray-300 text-gray-700 bg-transparent"
            >
            ...
            {currentStep === totalSteps ? (
              <Button
                ...
                className="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white font-semibold flex items-center gap-2"
              >
