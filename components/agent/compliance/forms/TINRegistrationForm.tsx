...
export function TINRegistrationForm({ agentId, onComplete, onCancel }: TINRegistrationFormProps) {
  ...
  const [ghanaCardBackFile, setGhanaCardBackFile] = useState<File | null>(null)
  ...
  const handleFileChange = (type: "signature" | "ghanaCard" | "ghanaCardBack", file: File | null) => {
    if (!file) return
  ...
  const removeFile = (type: "signature" | "ghanaCard" | "ghanaCardBack") => {
    if (type === "signature") {
  ...
  const handleSubmit = async () => {
    ...
    if (!signatureFile || !ghanaCardFile || !ghanaCardBackFile) {
      toast.error("Please upload signature, Ghana Card front, and Ghana Card back images")
      return
    }
    ...
    try {
      ...
      const ghanaCardUrl = await uploadImage(ghanaCardFile, "ghana_card_front")
      const ghanaCardBackUrl = await uploadImage(ghanaCardBackFile, "ghana_card_back")

      if (!signatureUrl || !ghanaCardUrl || !ghanaCardBackUrl) {
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="border-2 border-dashed border-emerald-300 rounded-lg p-4 mt-2">
                      {ghanaCardPreview ? (
                        <div className="relative">
                          <img
                        ...
                        <label className="flex flex-col items-center cursor-pointer">
                          <Upload className="h-8 w-8 text-emerald-600 mb-2" />
                          <span className="text-sm text-emerald-600">Click to upload Ghana Card Front</span>
                          <input
                            type="file"
                            accept="image/*"
                            className="hidden"
                      ...
                      {ghanaCardBackPreview ? (
                        <div className="relative">
                          ...
                          <Button
                            ...
                            size="sm"
                            className="absolute top-2 right-2"
                            onClick={() => removeFile("ghanaCardBack")}
                          >
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
                        ...
                        <label className="flex flex-col items-center cursor-pointer">
                          <Upload className="h-8 w-8 text-emerald-600 mb-2" />
                          <span className="text-sm text-emerald-600">Click to upload Ghana Card Back</span>
                          <input
                            type="file"
                            accept="image/*"
                            className="hidden"
                            onChange={(e) => handleFileChange("ghanaCardBack", e.target.files?.[0] || null)}
                          />
                        </label>
                      )}
                    </div>
