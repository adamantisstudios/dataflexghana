...
interface ProfessionalWritingSubmissionsListProps {
  agentId: string
  onUpdate: () => void
}

export function ProfessionalWritingSubmissionsList({ agentId, onUpdate }: ProfessionalWritingSubmissionsListProps) {
  ...
  const loadChatMessages = async (submissionId: string) => {
    try {
      const { data, error } = await supabase
        .from("professional_writing_chat")
        .select("*")
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const { error } = await supabase.from("professional_writing_chat").insert({
    ...
    return (
      <div className="space-y-4">
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-600 mt-1">Track the status of your professional writing requests</p>
        </div>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400">Click "New" to submit a writing request</p>
        </div>
      </div>
    )
  }
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {submissions.map((submission) => {
            ...
            const serviceType = getServiceTypeLabel(submission.service_type || "")
            const displayName = formData?.full_name || formData?.firstName || "Unknown"

            const formName = `${serviceType} - ${displayName}`

            return (
              <div
                key={submission.id}
                className="bg-white/50 backdrop-blur-sm rounded-lg p-4 border-l-4 border-pink-500 hover:bg-white/80 transition-all"
              >
                <div className="space-y-3">
        ...
        <DialogContent className="max-w-[95vw] sm:max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs sm:text-sm">
              View the details of your professional writing submission
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedSubmission && (
            <div className="space-y-3 sm:space-y-4 mt-4">
              ...
              <div>
                <span className="font-semibold text-base sm:text-lg">Request Information</span>
      ...
      {selectedSubmission && (
        <Dialog open={showChatDialog} onOpenChange={setShowChatDialog}>
          <DialogContent className="max-w-[95vw] md:max-w-2xl max-h-[90vh] flex flex-col">
            <DialogHeader>
              <DialogTitle className="text-base sm:text-lg md:text-xl font-bold text-pink-800">
                Chat - {getServiceTypeLabel(selectedSubmission.service_type || "")}
              </DialogTitle>
              <DialogDescription className="text-xs sm:text-sm text-pink-600">
            ...
            <div className="flex gap-2 pt-4 border-t border-pink-200">
              <Input
                ...
                className="flex-1 border-pink-200 focus:border-pink-500 text-xs sm:text-sm"
              />
              <Button onClick={handleSendMessage} className="bg-pink-600 hover:bg-pink-700 shrink-0">
