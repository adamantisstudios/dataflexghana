...
export function ResumeWritingForm({ agentId, onComplete, onCancel }: ResumeWritingFormProps) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (uploadedFile) {
        const fileName = `resume-${agentId}-${Date.now()}.pdf`
      ...
      toast.success("Resume writing request submitted successfully!")
      onComplete()
    } catch (error) {
          ...
          <Card>
            ...
            <CardContent>
              <Textarea
                ...
                onChange={handleInputChange}
                placeholder="Provide any special requests or specific details you want the admin to know about. For example: Resume needs to be tailored for Tech Industry, specific format preferences, etc."
                rows={4}
                className="text-xs"
              />
            </CardContent>
          ...
          <CardContent className="pt-6">
            <label className="block cursor-pointer">
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 hover:bg-gray-50 transition-colors">
                ...
                <p className="text-sm font-medium text-gray-700 mb-2">Click anywhere to upload your resume document</p>
