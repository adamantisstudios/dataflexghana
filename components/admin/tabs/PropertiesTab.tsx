...
export default function PropertiesTab({ getCachedData, setCachedData }: PropertiesTabProps) {
  ...
  useEffect(() => {
    const filtered = properties.filter((property) => {
      ...
      return matchesSearch && matchesCategory && matchesCurrency && matchesStatus
    })

    setFilteredProperties(filtered)
    setCurrentPage(1)
  }, [searchTerm, properties, categoryFilter, currencyFilter, statusFilter])
  ...
  const openPropertyDialog = (property?: Property) => {
    if (property) {
      ...
      setFormData({
        ...
        phone: property.contact_info?.phone || "0551999901",
