...
export default function AgentsTab({ getCachedData, setCachedData }: AgentsTabProps) {
  ...
  useEffect(() => {
    const loadAgents = async () => {
      ...
      try {
        // Get all agents with their wallet balances directly from the agents table
              ...
              try {
                ...
                if (Math.abs(liveWalletBalance - storedBalance) > 0.01) {
                  try {
                    ...
                    console.log(
                      `✅ Admin: Updated wallet balance for ${agent.full_name}: ${storedBalance} → ${liveWalletBalance}`,
                    )
                  } catch (error) {
                ...
                return {
                  ...
                  commission_balance: availableCommissions, // Now uses exact same calculation as all other pages
                }
              } catch (error) {
  ...
  useEffect(() => {
    ...
    if (agentsFilterAdmin !== "All Agents") {
      filteredByStatus = filtered.filter((agent) => {
        switch (agentsFilterAdmin) {
          ...
          case "Wallet Balance":
            // Filter agents with wallet balance > 0 and sort by balance (highest to lowest)
    ...
    // Sort by wallet balance if "Wallet Balance" filter is selected
    if (agentsFilterAdmin === "Wallet Balance") {
  ...
  const clearData = async () => {
    ...
    try {
      ...
      if (clearDataOptions.dataOrders) {
        // Only delete completed/processed orders, keep pending ones
        promises.push(
          supabase
            .from("data_orders")
            .delete()
            .in("status", ["completed", "confirmed", "canceled"])
            .eq("commission_paid", true) // Only delete if commission was already paid
      ...
      if (clearDataOptions.referrals) {
        // Only delete completed referrals where commission was paid
        promises.push(
          supabase
            .from("referrals")
            .delete()
      ...
      if (clearDataOptions.withdrawals) {
        // Only delete completed withdrawals (paid/rejected), never pending ones
        promises.push(
          supabase
            .from("withdrawals")
            .delete()
      ...
      // Refresh the current data to reflect changes
