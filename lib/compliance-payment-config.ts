...
export function formatCompliancePaymentInstructions(amount: number, formName?: string): string {
  ...
  return `Please pay ₵${amount.toFixed(2)}${label} to ${COMPLIANCE_ADMIN_PAYMENT.accountName} via Mobile Money: ${COMPLIANCE_ADMIN_PAYMENT.momoNumber}. Use your submission reference in the payment description. Contact ${COMPLIANCE_ADMIN_PAYMENT.altContact} if you need help.`
}
