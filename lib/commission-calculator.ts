...
export function getDisplayCommission(price: number, commissionRate: number): number {
  ...
  return Math.round(exactCommission * 100) / 100
}
...
export function validateCommission(
  ...
  const exactCommission = calculateExactCommission(price, commissionRate)
  const difference = Math.abs(exactCommission - targetCommission)
  ...
  return {
    ...
    exactCommission,
    targetCommission,
    calculatedCommission: exactCommission,
    difference,
  }
}
...
export function calculateTotalCommission(
  ...
  return orders.reduce((total, order) => {
    return total + calculateExactCommission(order.price, order.commissionRate)
  }, 0)
}
