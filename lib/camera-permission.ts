...
import { voiceCameraPermissionConstraints } from "@/lib/voice-video-utils"
...
export async function requestCameraAccess(_isMobile: boolean): Promise<CameraPermissionResult> {
  ...
  try {
    const stream = await navigator.mediaDevices.getUserMedia(voiceCameraPermissionConstraints())
