...
/**
 * Wallet transaction types — must match production CHECK constraint on
...
export const VALID_TRANSACTION_TYPES = DB_TRANSACTION_TYPES
...
export function isValidTransactionType(type: string): type is ValidTransactionType {
  return isValidDbTransactionType(type)
}
...
/**
 * Build a row for wallet_transactions INSERT (transaction_type only — no `type` column).
 */
...
export function createCommissionTransaction(
  ...
  bundlePrice: number,
  commissionRate = 10,
): { success: boolean; transaction?: Record<string, unknown>; skipped?: boolean; error?: string } {
  try {
