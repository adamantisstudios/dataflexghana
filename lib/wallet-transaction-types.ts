...
export function createSafeWalletTransaction(input: WalletTransactionInput): WalletTransactionInput {
  ...
  // FIXED: Build result with all fields, let database handle reference_code generation if not provided
  const result: WalletTransactionInput = {
    agent_id: input.agent_id.trim(),
    transaction_type: input.transaction_type as ValidTransactionType,
  ...
  // FIXED: Include reference_code if provided, otherwise let database generate it
  if (input.reference_code?.trim()) {
    result.reference_code = input.reference_code.trim()
  }

  // FIXED: Include payment_method if provided
...
export function generateSafeReferenceCode(transactionType: string, agentId?: string): string {
  try {
    ...
    // Use a default agent ID if not provided
    if (!agentId?.trim()) {
      agentId = "SYSTEM"
    }
