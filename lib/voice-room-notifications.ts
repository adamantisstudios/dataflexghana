...
/** Push in-app alert that the host is live now. */
export async function notifyAgentsHostIsLive(region: string, roomName: string): Promise<number> {
  const db = getAdminClient()
  const target = normalizeVoiceRegion(region)
  ...
  const title = "Host is live now!"
  const message = `Join the voice conference for ${region}. Open Voice Conference Room in your dashboard.`
  const now = new Date()
