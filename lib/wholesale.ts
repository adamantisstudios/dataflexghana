...
export async function createWholesaleProduct(
  ...
  try {
    ...
    console.log("Creating product with commission_value:", productData)
    ...
    try {
      ...
      if (error) {
        console.error("Database function error creating product:", error)
        ...
        if (
          ...
          console.log("Optimized function not found, using direct insert fallback")
      ...
      if (!data || data.length === 0) {
        console.log("No data returned from optimized function, using fallback")
        return await createWholesaleProductFallback(productData)
      }

      const createdProduct = Array.isArray(data) ? data[0] : data
      console.log("Product created successfully with optimized function:", createdProduct)
...
async function createWholesaleProductFallback(productData: any): Promise<WholesaleProduct> {
  try {
    console.log("Using fallback method for product creation")
    ...
    if (error) {
      console.error("Supabase error in fallback method:", error)
    ...
    console.log("Product created successfully with fallback method:", data)
    return data
  } catch (error) {
    if (error instanceof WholesaleError) {
      throw error
    }
    console.error("Error in fallback product creation:", error)
...
// Get all wholesale products (including inactive) for admin panel
export async function getAllWholesaleProducts(): Promise<WholesaleProduct[]> {
  try {
    const queryPromise = supabase.from("wholesale_products").select("*").order("created_at", { ascending: false })
      ...
      if (!referralsError && referrals) {
        referrals.forEach((r) => {
          commissionSources.push({
            ...
            source_name: r.services?.title || "Unknown Service",
